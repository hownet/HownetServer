...
{
	/// <summary>
	/// BAS_Customer_ContactStatus
	/// </summary>
	public partial class BAS_Customer_ContactStatus
	{
		private readonly Hownet.DAL.BAS_Customer_ContactStatus dal=new Hownet.DAL.BAS_Customer_ContactStatus();
		...
		{
			List<Hownet.Model.BAS_Customer_ContactStatus> li = DataTableToList(dt);
			int a=0;
			if (li.Count > 0)
			{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Customer_ContactStatus> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Customer_ContactStatus> DataTableToList(DataTable dt)
		{
			List<Hownet.Model.BAS_Customer_ContactStatus> modelList = new List<Hownet.Model.BAS_Customer_ContactStatus>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hownet.Model.BAS_Customer_ContactStatus model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hownet.Model.BAS_Customer_ContactStatus();
