...
namespace Hownet.BLL
{
	...
	{
		private readonly Hownet.DAL.BAS_Country dal=new Hownet.DAL.BAS_Country();
		public BAS_Country()
		{}
		#region  Method
		...
		{
			List<Hownet.Model.BAS_Country> li = DataTableToList(dt);
			int a=0;
			if (li.Count > 0)
			{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Country> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Country> DataTableToList(DataTable dt)
		{
			List<Hownet.Model.BAS_Country> modelList = new List<Hownet.Model.BAS_Country>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hownet.Model.BAS_Country model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hownet.Model.BAS_Country();
