...
namespace Hownet.BLL
{
	...
	{
		private readonly Hownet.DAL.BAS_Port dal=new Hownet.DAL.BAS_Port();
		public BAS_Port()
		{}
		#region  Method
		...
		{
			List<Hownet.Model.BAS_Port> li = DataTableToList(dt);
			int a=0;
			if (li.Count > 0)
			{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Port> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Port> DataTableToList(DataTable dt)
		{
			List<Hownet.Model.BAS_Port> modelList = new List<Hownet.Model.BAS_Port>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hownet.Model.BAS_Port model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hownet.Model.BAS_Port();
					if(dt.Rows[n]["CountryID"]!=null && dt.Rows[n]["CountryID"].ToString()!="")
					{
