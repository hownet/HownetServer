...
namespace Hownet.BLL
{
	...
	{
		private readonly Hownet.DAL.BAS_Container dal=new Hownet.DAL.BAS_Container();
		...
		{
			List<Hownet.Model.BAS_Container> li = DataTableToList(dt);
			int a=0;
			if (li.Count > 0)
			{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Container> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Container> DataTableToList(DataTable dt)
		{
			List<Hownet.Model.BAS_Container> modelList = new List<Hownet.Model.BAS_Container>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hownet.Model.BAS_Container model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hownet.Model.BAS_Container();
					...
					model.Dimension=dt.Rows[n]["Dimension"].ToString();
