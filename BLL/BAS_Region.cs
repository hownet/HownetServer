...
namespace Hownet.BLL
{
	...
	{
		private readonly Hownet.DAL.BAS_Region dal=new Hownet.DAL.BAS_Region();
		public BAS_Region()
		{}
		#region  Method
		...
		{
			List<Hownet.Model.BAS_Region> li = DataTableToList(dt);
			int a=0;
			if (li.Count > 0)
			{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Region> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hownet.Model.BAS_Region> DataTableToList(DataTable dt)
		{
			List<Hownet.Model.BAS_Region> modelList = new List<Hownet.Model.BAS_Region>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hownet.Model.BAS_Region model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hownet.Model.BAS_Region();
