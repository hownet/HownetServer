    ...
    public class Attendance
    {
        ...
        public string TrAtt()
        {
            try
            {
                ...
                DataRow[] drEm = BasicTable.dtEmployee.Select("(ID=" + CardID + ")");//是否有对应员工，没有就再次查询数据库，以添加新员工，如仍没有，见则转为查询是否为箱头卡
                if (drEm.Length == 0)
                {
                ...
                DataRow[] drs;
                if (dt.Hour < 6)
                    dtAR = bllAR.GetList("(EmployeeID=" + CardID + ") And (DateDay='" + dt.Date.AddDays(-1) + "')").Tables[0];
                else
                    dtAR = bllAR.GetList("(EmployeeID=" + CardID + ") And (DateDay='" + dt.Date + "')").Tables[0];
                ...
                {
                    DataTable dttt = bllAR.GetYesterday(CardID);
                ...
                mes = EmployeeName + dt.Hour.ToString() + "时" + dt.Minute.ToString() + "分" + mes;
                return mes;
            }
            catch (Exception ex)
            {
